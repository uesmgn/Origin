...
class PlaybackManager: NSObject {
    ...
    /// An enumeration of possible playback states that `PlaybackManager` can be in.
    ///
    /// - initial: The playback state that `PlaybackManager` starts in when nothing is playing.
    /// - playing: The playback state that `PlaybackManager` is in when its `AVPlayer` has a `rate` != 0.
    /// - paused: The playback state that `PlaybackManager` is in when its `AVPlayer` has a `rate` == 0.
    /// - interrupted: The playback state that `PlaybackManager` is in when audio is interrupted.
    enum playbackState {
    ...
    var state: PlaybackManager.playbackState = .initial
    ...
    /// The AVPlayerItem associated with PlaybackManager.asset.urlAsset
    fileprivate var playerItem: AVPlayerItem! {
        ...
        didSet {
            if playerItem != nil {
                ...
                NotificationCenter.default.addObserver(self, selector: #selector(PlaybackManager.handleAVPlayerItemDidPlayToEndTimeNotification(notification:)), name: .AVPlayerItemDidPlayToEndTime, object: playerItem)
            }
        }
    }
    ...
    var asset: Asset! {
        ...
        didSet {
            ...
            NotificationCenter.default.post(name: PlaybackManager.currentAssetDidChangeNotification, object: nil)
        }
    }
    ...
    override init() {
        super.init()
            ...
            NotificationCenter.default.addObserver(self, selector: #selector(PlaybackManager.handleAudioSessionInterruption(notification:)), name: .AVAudioSessionInterruption, object: AVAudioSession.sharedInstance())
        #endif
        
        // Add the Key-Value Observers needed to keep internal state of `PlaybackManager` and `MPNowPlayingInfoCenter` in sync.
    ...
    func nextTrack() {
        ...
        NotificationCenter.default.post(name: PlaybackManager.nextTrackNotification, object: nil, userInfo: [Asset.nameKey: asset.assetName])
    }
    
    func previousTrack() {
        guard asset != nil else { return }
        
        NotificationCenter.default.post(name: PlaybackManager.previousTrackNotification, object: nil, userInfo: [Asset.nameKey: asset.assetName])
    }
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        if keyPath == #keyPath(AVURLAsset.isPlayable) {
            ...
            NotificationCenter.default.post(name: PlaybackManager.playerRateDidChangeNotification, object: nil)
        }
        else {
